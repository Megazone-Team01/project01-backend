...
public class MeetingService {
    ...
    public void create( boolean isOnline, String name, int organizationId, int teacherId, int studentId, LocalDateTime startAt, LocalDateTime endAt,
        ...
        User student = userRepository.findById( studentId ).orElseThrow( () -> new CustomException("해당하는 학생이 존재하지 않습니다") );
    ...
    public void updateOnlineMeeting( int id, String name, LocalDateTime startAt, LocalDateTime endAt, String location ){
        OnlineMeeting meeting = onlineMeetingRepository.findById( id ).orElseThrow( () -> new CustomException("해당하는 면담이 존재하지 않습니다." ) );
    ...
    public void updateOfflineMeeting( int id, String name, LocalDateTime startAt, LocalDateTime endAt, Integer roomId ){
        OfflineMeeting meeting = offlineMeetingRepository.findById( id ).orElseThrow( () -> new CustomException("해당하는 면담이 존재하지 않습니다." ) );
        Room room = null;
        if( roomId != null ){
            room = roomRepository.findById( roomId ).orElseThrow( () -> new CustomException("해당하는 스터디룸이 존재하지 않습니다") );
        }
    ...
    public void delete( boolean isOnline, int id, int deletedBy ){
        Meeting meeting;
        if( isOnline ) meeting = onlineMeetingRepository.findById( id ).orElseThrow( () -> new CustomException("해당하는 면담이 존재하지 않습니다") );
        else meeting = offlineMeetingRepository.findById( id ).orElseThrow( () -> new CustomException("해당하는 면담이 존재하지 않습니다") );
